    ...
    const handleStatusChange = async (unidad: UnidadMedida, alta: boolean) => {
        try {
            ...
            setUnidades(prevUnidades =>
                prevUnidades.map(u =>
                    u.id === unidad.id ? updatedUnidad : u
                )
            );
        } catch (error) {
            console.log(error);
        ...
        return unidades.map((unidad, index) => (
            ...
            >
                ...
                <div className="d-flex gap-3">
                    ...
                    />
                    <Dropdown onSelect={(eventKey) => handleStatusChange(unidad, eventKey === 'alta')}>
        ...
        <Container className="text-center">
            ...
            <div className="d-flex justify-content-center my-3">
                <Button onClick={() => {
                    ...
                    setEditing(false);
                    setModalShow(true);
                }}>Crear Unidad de Medida</Button>
            </div>
            <ListGroup>
                {unidades.length > 0 ? renderUnidadesMedida(unidades) : <p>No hay unidades de medida</p>}
            </ListGroup>
            <UnidadMedidaModal
                show={modalShow}
                onHide={() => {
