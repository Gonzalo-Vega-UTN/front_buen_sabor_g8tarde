...
export const MisPedidosList = () => {
  ...
  const fetchPedidos = async () => {
    try {
      ...
      // Asegúrate de usar una propiedad adecuada del usuario para obtener los pedidos
      const pedidos = await PedidoService.obtenerPedidosCliente(user.id); // Suponiendo que `user.id` es el identificador
  ...
  useEffect(() => {
    fetchPedidos();
  }, [user]); // Asegúrate de volver a ejecutar la función si `user` cambia
