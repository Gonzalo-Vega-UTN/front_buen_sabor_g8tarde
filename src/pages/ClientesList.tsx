...
import FiltroClientes from "../components/Filtrado/FiltroCliente";

export default function ClientTable() {
  const [clientes, setClientes] = useState<Cliente[]>([]);
  const [searchedNombre, setSearchedNombre] = useState<string | undefined>(undefined);
  const [searchedApellido, setSearchedApellido] = useState<string | undefined>(undefined);

  const fetchClientes = async (nombre?: string, apellido?: string) => {
    try {
      const clientesFiltered = await ClienteService.obtenerClientes(nombre, apellido);
