...
export const PedidosCajero = () => {
    ...
    const fetchPedidos = async () => {
        try {
            const pedidosEnProceso = await PedidoService.obtenerPedidosXEstado(Estado.EnProceso);
            const pedidosPendientes = await PedidoService.obtenerPedidosXEstado(Estado.Pendiente);
    ...
    return (
        <>
            ...
            {pedidos.length > 0 ? pedidos.map((pedido) =>
                <Accordion style={{ maxWidth: "100%" }} key={pedido.id} className='mb-3'>
                    <Accordion.Item eventKey={String(pedido.id)}>
                        <Accordion.Header>
                            <div className="d-flex justify-content-between px-3 w-100">
                                ...
                                <div>{pedido.tipoEnvio ? pedido.tipoEnvio : 'Tipo de envío no disponible'}</div>
                        ...
                        <Accordion.Body>
                            <ListGroup as="ol" numbered>
                                {pedido.detallePedidos.map((detalle) =>
                                    <ListGroup.Item as="li" key={detalle.articulo.id}>{`${detalle.articulo.denominacion}: ${detalle.cantidad} ${detalle.articulo.unidadMedida?.denominacion} = $${detalle.articulo.precioVenta}`}</ListGroup.Item>
                                )}
                            </ListGroup>
                            <div className="d-flex justify-content-between mt-3">
                                {pedido.estado === Estado.EnProceso && (
                                    <>
                                        ...
                                        >
                                            {loadingPedidoId === pedido.id ? <Spinner animation="border" size="sm" /> : 'Rechazar Pedido'}
                                        </Button>
                                        <Button
                                            variant="success"
                                            onClick={() => actualizarEstado(pedido.id, Estado.Preparacion)}
                                            disabled={loadingPedidoId === pedido.id}
                                            aria-label={`Aceptar pedido ${pedido.id}`}
                                        >
                                            {loadingPedidoId === pedido.id ? <Spinner animation="border" size="sm" /> : 'Aceptar Pedido'}
                                        </Button>
                                    </>
                                )}
                                {pedido.estado === Estado.Pendiente && pedido.tipoEnvio === TipoEnvio.Delivery && (
                                    <Button
                                        variant="warning"
                                        onClick={() => actualizarEstado(pedido.id, Estado.EnDelivery)}
                                        disabled={loadingPedidoId === pedido.id}
                                        aria-label={`Marcar pedido ${pedido.id} como En Delivery`}
                                    >
                                        {loadingPedidoId === pedido.id ? <Spinner animation="border" size="sm" /> : 'Marcar como En Delivery'}
                                    </Button>
                                )}
                                {pedido.estado === Estado.Pendiente && pedido.tipoEnvio === TipoEnvio.TakeAway && (
                                    <Button
                                        variant="success"
                                        onClick={() => actualizarEstado(pedido.id, Estado.Entregado)}
                                        disabled={loadingPedidoId === pedido.id}
                                        aria-label={`Marcar pedido ${pedido.id} como Entregado`}
                                    >
                                        {loadingPedidoId === pedido.id ? <Spinner animation="border" size="sm" /> : 'Marcar como Entregado'}
                                    </Button>
                                )}
                            </div>
                        </Accordion.Body>
